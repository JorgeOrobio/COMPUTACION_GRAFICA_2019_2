...
def diujar_circulo(pantalla, ls_puntos):
        pg.draw.circle(pantalla,BLANCO,ls_puntos,1)

##LAS TUPLAS NO SE PUEDEN CAMBIAR!!!
def trasladar_circulo(pantalla,ls_puntos, escalado ):
        ...
        for event in pg.event.get():
            ...
            if event.type == pg.MOUSEBUTTONDOWN:
                if pg.MOUSEBUTTONDOWN == 1:
                    ls_puntos=event.pos
                    print(1)
                diujar_circulo(pantalla,ls_puntos)
                if pg.MOUSEBUTTONDOWN == 4:
                    ls_puntos=rotar_punto_antihorario(pantalla,ls_puntos,5)
                if pg.MOUSEBUTTONDOWN == 5:
                    ls_puntos=rotar_punto_horario(pantalla,ls_puntos,5)
                # trasladar_circulo(pantalla,ls_puntos,trasladado2)
                # trasladar_circulo(pantalla,ls_puntos,trasladado)
                pg.display.flip()
