...
if __name__ == '__main__':
    ...
    while (not fin) and (not fin_de_juego):
        ...
        # CONTROL DEL SISTEMA
        # LIMITES CON  UN MURO
        lsc=pg.sprite.spritecollide(j,bloques,False)
        for b in lsc:
            if(j.rect.right > b.rect.left) and (j.velx >0):
                j.rect.right=b.rect.left
                j.velx=0
            if(j.rect.left < b.rect.right) and (j.velx <0):
                j.rect.left=b.rect.right
                j.velx=0
            if(j.rect.bottom > b.rect.top) and (j.vely >0):
                j.rect.bottom=b.rect.top
                j.vely=0
            if(j.rect.top < b.rect.bottom) and (j.vely<0):
