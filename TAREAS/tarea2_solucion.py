...
def dibujar_suma_de_linea1(pantalla,posicion):
    x,y = transformacion_cartesiano_pantalla(posicion[0],posicion[1])
    pg.draw.line(pantalla,VERDE,ORIGEN,[x,y])
def sumar_xs(x1,x2):
    return x1 + x2
def sumar_ys(y1,y2):
    return y1 + y2

def sumar_puntos(punto1,punto2):
    x=sumar_xs(punto1[0],punto2[0])
    y=sumar_ys(punto1[1],punto2[1])
    return x,y
...
if __name__ == '__main__':
    ...
    while not fin:
        ...
        x,y = sumar_puntos([60,80],[100,30])
        dibujar_suma_de_linea1(pantalla,[x,y])
        for e in event:
            if e.type == pg.QUIT:
                fin = True
